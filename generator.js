...
const STANDALONE_BASE_URL = "https://asli180606-svg.github.io/man-bro-r/generator.html";
...
async function loadFx(){
  // 1) Önce KENDİ sitemizdeki (aynı adres/origin) kur.json dosyasını dene —
  //    bu dosya GitHub Actions ile her gün otomatik güncelleniyor. Aynı
  //    origin'den geldiği için CORS/kurumsal güvenlik duvarı sorunu olmaz;
  //    broşür sayfası zaten açılabiliyorsa bu dosya da açılabilir.
  try{
    const res = await fetch('kur.json', { cache: 'no-store' });
    if(res.ok){
      const data = await res.json();
      if(data && data.eur) return { eur: data.eur, date: data.date || '—', ok: true };
    }
  }catch(e){ /* dosya yoksa ya da okunamazsa aşağıdaki yola düş */ }

  // 2) Olmazsa canlı kur servisini dene (bazı ağlarda engellenmiş olabilir).
  try{
    const res = await fetch('https://api.frankfurter.app/latest?from=TRY&to=EUR');
    const data = await res.json();
    return { eur: 1/data.rates.EUR, date: data.date, ok:true };
  }catch(e){
    return { eur: 55.11, date: 'sabit değer', ok:false };
  }
}
...
// bir üst boyutu deniyoruz. errorLevel='Q' (yüksek hata düzeltme) kullanıyoruz;
// bu, kod biraz daha yoğun olsa da, basılı/küçük ekrandaki QR'ı telefon
// kamerasının yanlış okuması ihtimalini belirgin şekilde azaltıyor.
function buildQrSvg(text, errorLevel){
  if(typeof qrcode === 'undefined') return null;
  const level = errorLevel || 'Q';
  for(let tn = 1; tn <= 40; tn++){
    try{
      const qr = qrcode(tn, level);
      qr.addData(text);
      qr.make();
...
function buildSharePayload(maxPhotos, includeSpecs){
  const n = maxPhotos == null ? 6 : maxPhotos;
  const withSpecs = includeSpecs == null ? true : includeSpecs;
  const payload = {
    title: currentListing.title, priceRaw: currentListing.priceRaw, url: currentListing.url,
    photos: galleryState.slice(0, n).map(s => (allPhotoPool[s.poolIndex] || {}).url), photoLabels: galleryState.slice(0, n).map(s => s.label)
  };
  if(withSpecs) payload.specs = currentListing.specs;
  return payload;
}

function buildShareLink(maxPhotos, includeSpecs){
  return STANDALONE_BASE_URL + '#d=' + encodeShareData(buildSharePayload(maxPhotos, includeSpecs));
}

function renderQr(){
  const container = document.getElementById('qrRoot');
  if(!container || !currentListing) return;
  // Aynı formatı (6 foto + tüm özellikler) koruyoruz — veri azaltmak yerine
  // QR'ı büyük ve düşük yoğunlukta (L seviyesi = en az ek veri) tutup fiziksel
  // boyutunu büyütmeyi tercih ettik (bkz. generator.html .qr-col svg).
  const svg = buildQrSvg(buildShareLink(), 'L');
